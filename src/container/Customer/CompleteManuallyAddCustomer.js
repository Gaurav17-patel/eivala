...
class CompleteManuallyAddCustomer extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      stickHeaderHeight: 0,
  ...
  userCurrentValueData(item, index) {
    this.setState({
      currentIndexDataValue: index,
      isDomicileValue: item.id,
      isDomicile: index === 1 ? false : true,
    });
  }
    ...
    return (
      <View style={styles.filterOptionRenderMainView}>
        <TouchableOpacity
          ...
          onPress={() => this.userCurrentValueData(item, index)}
        >
          <Image
            source={
  ...
  socialValidationCheck() {
    ...
    if (
      ...
      this.addNewCustomerSocialAPICall();
    }
  }
  ...
  naturalValidationCheck() {
    ...
    if (
      ...
      this.addNewCustomerNaturalAPICall();
    }
  }

  addNewCustomerSocialAPICall() {
    const isConnected = NetworkUtils.isNetworkAvailable();
    const { userCountryCode, postCodeAddress, CFRValue } = this.state;
    if (isConnected) {
      ...
      this.props.manuallyAddCustomerAPI(formData).then(async () => {
        const { manuallyAddCustomer, error } = this.props;
        let errorData;
        {
          error ? (errorData = JSON.parse(error)) : null;
        }
        if (manuallyAddCustomer) {
          this.setState({ isLoading: false });
          FunctionUtils.showToast(manuallyAddCustomer.message);
  ...
  addNewCustomerNaturalAPICall() {
    ...
    if (isConnected) {
      ...
      formData.append('domicile_equal_residence', isDomicileValue);
      ...
      <View style={{ marginHorizontal: moderateScale(15) }}>
        ...
        <View style={styles.secondBoxMainView}>
          <View style={styles.commonCalenderBoxHeight}>
            <RNPickerSelect
              placeholder={{ label: strings.domCountry, value: null }}
              onValueChange={(value) =>
        ...
        <View>
          {italyCountryId === 107 ? (
            <View style={styles.secondBoxMainView}>
              <View style={styles.commonCalenderBoxHeight}>
                <RNPickerSelect
                  placeholder={{ label: strings.domProvince, value: null }}
                  onValueChange={(value) =>
          ...
          <SafeAreaView>
            ...
            >
              ...
              <View style={styles.secondBoxMainView}>
                <View style={styles.commonCalenderBoxHeight}>
                  <RNPickerSelect
                    placeholder={{ label: strings.ReState, value: null }}
...
const mapStateToProps = (state) => {
  return {
    ...
    manuallyAddCustomer: state.customerReducer.manuallyAddCustomer, //accessing the redux state
